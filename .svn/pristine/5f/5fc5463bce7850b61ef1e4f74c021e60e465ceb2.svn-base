...
public class CreditorReportBean extends AppBackingBean {
	...
	private BankAcctViewVO bankAcctVO;
	...
	public void handleSelectReportType() {
		try {
			...
			if (reportType.equals("CA") || reportType.equals("CC") || reportType.equals("CD") || reportType.equals("CF")) {
				...
				if (CollectionUtils.isNotEmpty(finPeriodList)) {
					...
					if (reportType.equals("CA") || reportType.equals("CC")) {
						searchParamVO.setObj4(finPeriodList.get(0).getDtStart());
						searchParamVO.setObj5(finPeriodList.get(finPeriodList.size() - 1).getDtEnd());
					}
				} else {
			...
			if (reportType.equals("CA") || reportType.equals("CC") || reportType.equals("CD") || reportType.equals("CE")) {
	...
	public void printReport(String xlsOrPDf) {
		try {
			if (reportType.equals("CA")) {
				printBillDetailBySupplier(xlsOrPDf);
			} else if (reportType.equals("CC")) {
				printBillUnpaidDetailBySupplier(xlsOrPDf);
			} else if (reportType.equals("CB")) {
				printSupplierList(xlsOrPDf);
			} else if(reportType.equals("CD")){
				printBankPaymentByPayee(xlsOrPDf);
			} else if(reportType.equals("CE")){
				printAPDetailAgingBySupplier(xlsOrPDf);
			} else if(reportType.equals("CF")){
				printPurchasesInAdvanceRpt(xlsOrPDf);
			}
	...
	/*************************************************************************************************************************************
	 * Creditor Report Print *
	 *************************************************************************************************************************************/
	/**
	 * Printing Bill Detail By Supplier
	 */
	public void printBillDetailBySupplier(String xlsOrPDf) {
		try {
			trackingLogUtils.startLogs();
			
			HashMap<String, Object> map = reportTitle();
			map.put("dateFr", (Date) searchParamVO.getObj2());
			map.put("dateTo", (Date) searchParamVO.getObj3());
			map.put("depDateFr", (Date) searchParamVO.getObj4());
			map.put("depDateTo", (Date) searchParamVO.getObj5());
			map.put("glCode", glCodeChecked);

			if (supplierVO.getId() != null && supplierVO.getId() != 0) {
				searchParamVO.setObj6(supplierVO.getId());
			} else {
				searchParamVO.setObj6(null);
			}
			searchParamVO.setObj7(glCodeChecked);

			List<ExOrderBillVO> list = (List<ExOrderBillVO>) acctRptBO.getAccountReport(getSessionInfoBean().getCompanyVO().getId(), searchParamVO, reportType);

			if (CollectionUtils.isEmpty(list)) throw new BusinessException(CommonErrConstant.ERR_NO_RESULT);
			
			String jasperFileName = CommonConstant.JAS_RPT_BILL_BY_SUPPLIER;
			String reportName = CommonConstant.PDF_RPT_BILL_BY_SUPPLIER;
			...
			trackingLogUtils.endLogs("printBillDetailBySupplier");
		}
	}
	
	/**
	 * Printing Bill Unpaid Detail By Supplier
	 */
	public void printBillUnpaidDetailBySupplier(String xlsOrPDf) {
		try {
			trackingLogUtils.startLogs();
			...
			String jasperFileName = CommonConstant.JAS_RPT_BILL_UNPAID_BY_SUPPLIER;
			String reportName = CommonConstant.PDF_RPT_BILL_UNPAID_BY_SUPPLIER;
	...
	/**
	 * Printing Customer List Report
	 */
	public void printSupplierList(String xlsOrPDf) {
		try {
			trackingLogUtils.startLogs();
			...
			String reportTitle = "", reportSubName = "", sundryOrTrade = "";
			if (orderType.equalsIgnoreCase("SuN")) {
				reportTitle = "SUPPLIER NAME";
				reportSubName = "SupplierName";
			} else {
				reportTitle = "SUPPLIER CODE";
				reportSubName = "SupplierCode";
			}
			map.put("sorting", reportTitle);

			if (StringUtils.isNotEmpty(supplierType)) {
				searchParamVO.setObj4(supplierType);
			} else {
				searchParamVO.setObj4(null);
			}
			
			if (supplierType.equals("TC")) {
				sundryOrTrade = "Trade Creditors";
			} else if (supplierType.equals("TSC")) {
				sundryOrTrade = "Sundry Creditors";
			}
			map.put("supplierType", sundryOrTrade);

			// Order type
			searchParamVO.setObj7(orderType);

			List<SupplierVO> list = (List<SupplierVO>) acctRptBO.getAccountReport(getSessionInfoBean().getCompanyVO().getId(), searchParamVO, reportType);

			if (CollectionUtils.isEmpty(list)) throw new BusinessException(CommonErrConstant.ERR_NO_RESULT);
			
			String jasperFileName = CommonConstant.JAS_RPT_SUPP_LIST_BY_SORTING;
			String reportName = CommonConstant.PDF_RPT_SUPP_LIST_BY_SORTING.replace("[SORTING]", reportSubName);
			...
			trackingLogUtils.endLogs("printSupplierList");
		}
	}

	/**
	 *  Bank Payment By Payee
	 */
	public void printBankPaymentByPayee(String xlsOrPDf){
		try{
			trackingLogUtils.startLogs();
			
			String sundryOrTrade = "";
			HashMap<String, Object> map = reportTitle();
			map.put("fromDate", (Date) searchParamVO.getObj2());
			map.put("toDate", (Date) searchParamVO.getObj3());
			
			if (supplierVO.getId() != null && supplierVO.getId() != 0) {
				searchParamVO.setObj4(supplierVO.getId());
			} else {
				searchParamVO.setObj4(null);
			}
			
			if (StringUtils.isNotEmpty(supplierType)) {
				searchParamVO.setObj5(supplierType);
			} else {
				searchParamVO.setObj5(null);
			}
			
			if (bankAcctVO.getId() != null && bankAcctVO.getId() != 0) {
				searchParamVO.setObj6(bankAcctVO.getId());
			} else {
				searchParamVO.setObj6(null);
			}
			...
			if (supplierType.equals("TC")) {
				sundryOrTrade = "Trade Creditors";
			} else if (supplierType.equals("SC")) {
				sundryOrTrade = "Sundry Creditors";
			}
			map.put("supplierType", sundryOrTrade);
			
			List<CashBookVO> list = (List<CashBookVO>) acctRptBO.getAccountReport(getSessionInfoBean().getCompanyVO().getId(), searchParamVO, reportType);

			if (CollectionUtils.isEmpty(list)) throw new BusinessException(CommonErrConstant.ERR_NO_RESULT);
			
			String jasperFileName = CommonConstant.JAS_RPT_BANK_PAYMENT_BY_PAYEE;
			String reportName = CommonConstant.PDF_RPT_BANK_PAYMENT_BY_PAYEE;
			...
			trackingLogUtils.endLogs("printBankPaymentByPayee");
		}
	}
	 ...
	 */
	public void printAPDetailAgingBySupplier(String xlsOrPDf){
		try{
			trackingLogUtils.startLogs();
			...
			String jasperFileName = CommonConstant.JAS_RPT_AP_DETAIL_AGING_BY_SUPPLIER;
			String reportName = CommonConstant.PDF_RPT_AP_DETAIL_AGING_BY_SUPPLIER;
			...
			trackingLogUtils.endLogs("printAPDetailAgingBySupplier");
		}
	}
	
	/**
	 * Printing Purchases In Advance Report
	 */
	public void printPurchasesInAdvanceRpt(String xlsOrPDf) {
		try {
			...
			List<ExOrderBillVO> salesInAdvanceList = (List<ExOrderBillVO>) acctRptBO.getAccountReport(getSessionInfoBean().getCompanyVO().getId(), searchParamVO, reportType);
			
			if (CollectionUtils.isEmpty(salesInAdvanceList)) throw new BusinessException(CommonErrConstant.ERR_NO_RESULT);
			
			String jasperFileName = CommonConstant.JAS_RPT_PURC_ADV;
			String reportName = CommonConstant.PDF_RPT_PURC_ADV;
	...
	public void clearDepartureDate() {
	    searchParamVO.setObj4(null); // Dep Dt From
	    searchParamVO.setObj5(null); // Dep Dt To
	}
	 ...
	 */
	public BankAcctViewVO getBankAcctVO() {
		return bankAcctVO;
	}
	 ...
	 */
	public void setBankAcctVO(BankAcctViewVO bankAcctVO) {
