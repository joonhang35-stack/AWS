package com.bcs.zsg.sales.web.api;
...
@Controller
@RequestMapping("/v1")
public class EmailPaymentController {
	...
	@RequestMapping(value="/emailPaymentResult", method=RequestMethod.POST, produces="text/plain;")
	public void getEmailPaymentResult(HttpServletRequest request, HttpServletResponse response) {
		PrintWriter out = null;
		
		try {
			logger.error("----- Ipay88 Message : " + request.getParameter("ErrDesc"));
			logger.error("----- Ipay88 Status : " + request.getParameter("Status") + " / " + request.getRemoteAddr());
			
			out = response.getWriter();
			
			// 0 means unsuccessful, 1 means success
			if (StringUtils.equals(request.getParameter("Status") , "1")) {
				String tempRefNo = request.getParameter("RefNo");
				...
				InvoiceEmailPaymentVO invoiceEmailPaymentVO = invoiceEmailPaymentBO.getInvoiceEmailPayment(params);
				
				if (invoiceEmailPaymentVO != null) {
					if (CommonConstant.EMAIL_PMNT_PENDING.equals(invoiceEmailPaymentVO.getPaymentStatus())
							|| CommonConstant.EMAIL_PMNT_SUCCESS.equals(invoiceEmailPaymentVO.getPaymentStatus())) {
						
						if (CommonConstant.EMAIL_PMNT_PENDING.equals(invoiceEmailPaymentVO.getPaymentStatus())) {
							invoiceEmailPaymentVO.setPaymentStatus(CommonConstant.EMAIL_PMNT_SUCCESS);
							invoiceEmailPaymentVO.setTransId(request.getParameter("TransId"));
							invoiceEmailPaymentVO.setAuthCode(request.getParameter("AuthCode"));
							invoiceEmailPaymentVO.setCcName(request.getParameter("CCName"));
							invoiceEmailPaymentVO.setCcNo(request.getParameter("CCNo"));
							invoiceEmailPaymentVO.setsBankName(request.getParameter("S_bankname"));
							invoiceEmailPaymentVO.setsCountry(request.getParameter("S_country"));
						...
						if (invoiceVO != null) {
							if (StringUtils.equals(invoiceVO.getStatusCd(), CommonConstant.STATUS_CD_VOID) || 
								...
								logger.error("[Status]" + request.getParameter("Status") + "[RefNo]" + tempRefNo + "[invoice_status]" + invoiceVO.getStatusCd());
								out.println("FAILED");
								return;
							}
							...
							paymentVO.setPmntFor(invoiceEmailPaymentVO.getIpayConfigComment());
							paymentVO.setAmount(invoiceEmailPaymentVO.getTotalPaymentAmt().doubleValue());
							...
							if (!financialClosed) {
								if (ipayConfigVO != null) {
									if (invoiceEmailPaymentVO.getAdminCharges().compareTo(BigDecimal.ZERO) > 0) {
							...
							// Check if financial close then create a new invoice to store admin charges
								...
								if (invoiceEmailPaymentVO.getAdminCharges().compareTo(BigDecimal.ZERO) > 0) {
									...
									InvoicePaymentVO tmpPyVO = new InvoicePaymentVO();
									tmpPyVO.setPmntDt(new Date());
									tmpPyVO.setPmntTypeCd("ipay88");
									tmpPyVO.setRefNo(invoiceEmailPaymentVO.getTransId());
									tmpPyVO.setPmntFor("Admin Charges - " + invoiceEmailPaymentVO.getIpayConfigComment());
									tmpPyVO.setAmount(invoiceEmailPaymentVO.getAdminCharges().doubleValue());
									tmpPyVO.setPmntIssuedBy("SYSTEM");
									tmpPyVO.setRecievedFr(paymentVO.getRecievedFr());
									tmpPyVO.setCreatedBy("SYSTEM");
									tmpPyVO.setUpdatedBy("SYSTEM");
									
									// Set original invoice amount without admin charges
									paymentVO.setAmount(invoiceEmailPaymentVO.getPaymentAmt().doubleValue());
									tempVO.getInvoicePaymentList().add(tmpPyVO);
									...
									// update email payment admin charges invoice id
									invoiceEmailPaymentVO.setIdInvoiceAdminCharges(tempVO.getId());
									invoiceEmailPaymentBO.updateInvoiceEmailPaymentBySystem(invoiceEmailPaymentVO);
									
									// add admin charges invoice reference to epl invoice
									invoiceReferenceVO = new InvoiceReferenceVO();
									invoiceReferenceVO.setInvRefId(tempVO.getId());
									invoiceReferenceVO.setInvRefCode(tempVO.getCode());
									invoiceVO.setGenAUDINvoiceRefVOList(new GenAddUpdDelVO<InvoiceReferenceVO>());
									invoiceVO.getGenAUDINvoiceRefVOList().getAddList().add(invoiceReferenceVO);
								}
							}
							...
							logger.error("[Status]" + request.getParameter("Status") + "[RefNo]" + tempRefNo + " | invoiceVO=null");
							out.print("FAILED");
						}
					} else {
						logger.error("[Status]" + request.getParameter("Status") + "[RefNo]" + tempRefNo + "[email_payment_status]" + invoiceEmailPaymentVO.getPaymentStatus());
						out.print("FAILED");
					}
				} else {
					logger.error("[Status]" + request.getParameter("Status") + "[RefNo]" + tempRefNo + " | invoiceEmailPaymentVO=null");
					out.print("FAILED");
				}
			} else {
				logger.error("[Status]" + request.getParameter("Status") + "[ErrDesc]" + request.getParameter("ErrDesc"));
				out.print("FAILED");
			}
		} catch (Exception e) {
	...
	public boolean getFinPeriodClosedStatus(InvoiceVO invoiceVO) throws Exception {
		boolean rtn = false;
		try {
	...
	private void calTtlAmt(InvoiceVO invoiceVO) throws Exception {
		try {
