...
@WebServlet("/getGroupPackageGolf")
public class getGroupPackageGolf extends WebServiceServlet {

	   /**
	     * @see HttpServlet#HttpServlet()
	     */
	    public getGroupPackageGolf() {
	        super();
		...
		protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
			...
			try{ 
			    ...
			    Statement stmt = conn.createStatement();
			      ...
			      sql = "select d.id,d.num_days,d.num_nights,d.description,a.code,d.tour_status_cd,d.dt_dep,d.full_twn as adult_twin, d.full_sgl as adult_single, d.dt_dep,d.full_ctw as ctw, d.full_ceb as cwb, d.full_cnb as cnb,d.misc_adt as misc,  d.misc_chd as miscChd, it.name as itineryEn,it2.name as itineryCn , pkg.name_en as pkgName from tour_dep d inner join airline a on d.id_airline = a.id inner join tour_pkg pkg on pkg.id = d.id_tour_pkg  left join tour_itinery it on it.id_tour_dep = d.id and it.lang_cd = 'EN' and it.type_cd = 'CUST' left join tour_itinery it2 on it2.id_tour_dep = d.id and it2.lang_cd = 'ZH' and it2.type_cd = 'CUST'";
			      sql += " where d.tour_status_cd IN ('A','F','L') and d.status_cd='AC'  and dt_dep > now() and pkg.status_cd ='AC' and pkg.type_cd='TOUR'";
