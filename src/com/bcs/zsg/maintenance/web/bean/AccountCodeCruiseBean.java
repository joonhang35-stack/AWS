...
public class AccountCodeCruiseBean extends AppBackingBean {
	...
	public void refreshList() {
		try {
		 	tourDepVOList = accountCodeBO.getTourDepCruiseListWithItem();
...
//				}
		...
		try {
			if(validateAccountCode()) {
				...
				for(TourDepItemVO itemVO : tourDepItemVOList) {
					if (ProductConstant.TOUR_DEP_ITM_TYPE_TOUR.equals(itemVO.getTypeCd()) && itemVO.isCabinItem()) {
						TourCruiseCabinVO cabinVO = new TourCruiseCabinVO();
						cabinVO.setId(itemVO.getId());
						cabinVO.setIdAcct(itemVO.getAcctId());
						cabinVO.setIdTourDep(itemVO.getIdTourDep());
						cabinVO.setIsLock(itemVO.getIsLock());
						tourCruiseCabinBO.updateTourCruiseCabinAcctCode(cabinVO);
					} else tourDepItemAUDVO.getUpdList().add(itemVO);
				}
				accountCodeBO.updAccountCode(tourDepItemAUDVO);

				handleAccountDepSelect(tourDepVO, companyVO.getId().toString());
	...
	public void handleAccountDepSelect(TourDepartureVO vo, String companyId) {
		try {
			...
			if(tourThemeVO.getIdCountry() != null && tourThemeVO.getIdCountry() != 0) {
			...
			if(companyList.size() > 0) {
				...
				List<TourCruiseCabinVO> cabinList = tourCruiseCabinBO.getTourCruiseCabinList(tourDepVO.getId(), getSessionInfoBean().getCompanyVO().getId());
				TourDepItemVO depItemVO;
				for (TourCruiseCabinVO cabinVO : cabinList) {
					depItemVO = new TourDepItemVO();
					depItemVO.setId(cabinVO.getId());
					depItemVO.setCode(cabinVO.getCruiseCabinCd());
					depItemVO.setAcctId(cabinVO.getIdAcct());
					depItemVO.setIdTourDep(cabinVO.getIdTourDep());
					depItemVO.setIdCompany(tempLong);
					depItemVO.setDesc(cabinVO.getCruiseCabinDesc());
					depItemVO.setTypeCd(ProductConstant.TOUR_DEP_ITM_TYPE_TOUR);
					depItemVO.setGrouping(0);
					depItemVO.setCabinItem(true);
					depItemVO.setIsLock(cabinVO.getIsLock());
					tourDepItemVOList.add(depItemVO);
				}

				populateAccList(tempLong);

				List<AccountCodeConfigVO> accountCodeConfigVOList = accountCodeBO.getAccountCodeConfigList(ProductConstant.TYPE_CRUISE);

				// Default The account if not set before
				for (TourDepItemVO itemVO : tourDepItemVOList) {
					if (ProductConstant.TOUR_DEP_ITM_TYPE_CRUISE.equals(itemVO.getTypeCd()))
						itemVO.setGrouping(1);
