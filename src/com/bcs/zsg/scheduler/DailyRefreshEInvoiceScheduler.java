...
public class DailyRefreshEInvoiceScheduler extends AppBackingBean implements Job {
	...
	public DailyRefreshEInvoiceScheduler() {
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		...
		System.out.println("DailyRefreshEInvoiceScheduler.execute() @ " + new Date());
		...
		try {
			...
			for (int j = 1; j < 4; j++) {
				...
				cal.add(Calendar.DATE, -j); // subtract 1 day
				Date getRecentDocumentsDate = DatesUtils.getDayTimeStart(cal.getTime()).getTime();
				
				CompanyVO companyVO = corporateProfileBO.getCompanyDetails(1L);

				List<ApiDocumentQueryResponseVO> responseList = eInvoiceBO.getRecentDocuments(getRecentDocumentsDate, companyVO);

				processApiDailySnapshotTransactions(companyVO.getId(), getRecentDocumentsDate);
			}
		} catch (Throwable t) {
			t.printStackTrace();
		} finally {
			trackingLogUtils.endLogs("DailyRefreshEInvoiceScheduler.execute()");
		}
	}
	
	private void processApiDailySnapshotTransactions(Long idCompany, Date dtTrans) throws BusinessException {
		List<EInvoiceApiDailySnapshotTransVO> transList = eInvoiceBO.getApiDailySnapshotTransList(idCompany, dtTrans);
		
		System.out.println("CYY transList.size(): " + transList.size());
		
		for (EInvoiceApiDailySnapshotTransVO transVO : transList) {
			EInvoiceDocumentVO docVO = eInvoiceBO.getEInvoiceByUUID(transVO.getUuid());
			if (docVO != null) {
//				System.out.println("CYY docVO.getRefNo(): " + docVO.getRefNo());
				try {
					EInvoiceUtils.copyDetails(docVO, transVO);
					eInvoiceBO.updEInvoiceDocumentVO(docVO, null);
					
					eInvoiceBO.updateApiDailySnapshotTransStatus(transVO.getId(), EInvoiceConstant.EXISTS_STATUS_YES, CommonConstant.EMAIL_PMNT_SUCCESS, null);
					
//					if (StringUtils.equalsIgnoreCase(docVO.getStatusCode(), EInvoiceConstant.E_INV_STATUS_INVALID)) {
//						try {
//							EInvoiceUtils eInvoiceUtils = new EInvoiceUtils(eInvoiceBO);
//							CompanyVO companyVO = corporateProfileBO.getCompanyDetails(1L);
//							String accessToken = eInvoiceBO.getAccessToken(companyVO, true);   
//							EInvoiceDocumentVO eInvoiceDocumentVO = eInvoiceUtils.updDocumentDetails(docVO.getDocumentUuid(), accessToken);
//							if (StringUtils.isNotBlank(eInvoiceDocumentVO.getDocStatusReason()))	eInvoiceBO.updateApiDailySnapshotTransStatus(transVO.getId(), null, null, eInvoiceDocumentVO.getDocStatusReason());
//							eInvoiceSchedulerBO.notifyInvalidEInvoice(eInvoiceDocumentVO);
//						} catch (Exception e) {
//							e.printStackTrace();
//						}
//					}
				} catch (Exception e) {
					eInvoiceBO.updateApiDailySnapshotTransStatus(transVO.getId(), EInvoiceConstant.EXISTS_STATUS_YES, CommonConstant.EMAIL_PMNT_FAILED, null);
					e.printStackTrace();
				}
			} else {
				eInvoiceBO.updateApiDailySnapshotTransStatus(transVO.getId(), EInvoiceConstant.EXISTS_STATUS_NO, CommonConstant.EMAIL_PMNT_FAILED, null);
			}
